...
function openServiceModal(serviceType) {
  // Nếu là dịch vụ money, mở modal quyên góp tiền
  if (serviceType === 'money') {
    const moneyModal = document.getElementById('moneyDonationModal');
    if (moneyModal) {
...
function handleContactForm(e) {
  ...
  console.log("Cảm ơn bạn đã liên hệ! Chúng tôi sẽ phản hồi sớm nhất có thể.")
  e.target.reset()
}
...
const translations = {
  vi: {
    ...
    service_money_title: "Hỗ Trợ Tiền",
    service_money_desc: "Quyên góp tiền để hỗ trợ các hoàn cảnh khó khăn và dự án từ thiện.",
    service_money_direct: "Trực tiếp",
    service_money_transparent: "Minh bạch",
    service_money_impact: "Tác động",
    service_money_btn: "Quyên Góp Tiền",
    // How it works
  ...
  en: {
    ...
    service_money_title: "Money Support",
    service_money_desc: "Donate money to support difficult circumstances and charity projects.",
    service_money_direct: "Direct",
    service_money_transparent: "Transparent",
    service_money_impact: "Impact",
    service_money_btn: "Donate Money",
    // How it works
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (registerForm) {
    registerForm.onsubmit = function(e) {
      ...
      console.log('Đăng ký thành công! Bạn có thể đăng nhập.');
      registerModal.style.display = 'none';
      registerForm.reset();
    };
  }
...
// ===== GEMINI AI CHATBOT =====
const GEMINI_API_KEY = 'AIzaSyDOQBrWrW9vAr_WbT5W4ovgLXbr5o8IpCw';
const GEMINI_API_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent';

let chatHistory = [];

// Tinh chỉnh AI cho trả lời thông minh hơn
...
function getSelectedLanguageLabel() {
  const lang = localStorage.getItem('currentLanguage') || 'vi';
  switch (lang) {
...
// Lấy ngữ cảnh từ trang để AI trả lời đúng chủ đề
function getSiteContext() {
  try {
    const importantSelectors = [
      '.hero-title', '.hero-description', '.services', '.centers', '.about', '.contact',
      '#services', '#centers', '#about', '#contact', '.stories-section'
    ];
    let text = '';
    ...
    // Giới hạn độ dài ngữ cảnh để tiết kiệm token
...
const announcements = [
  {
    id: 'blood_drive_sep',
    title: 'Chương trình hiến máu Tháng 9',
    date: '2025-09-20',
    summary: 'Tổ chức tại Quận 1, TP.HCM, từ 8:00 đến 16:00. Ưu tiên nhóm máu O.',
    tags: ['hiến máu', 'sự kiện']
  },
  {
    id: 'clothes_campaign',
    title: 'Phát động quyên góp áo ấm cho vùng cao',
    date: '2025-10-01',
    summary: 'Nhận quần áo mùa đông cho trẻ em từ 01/10 đến 31/10.',
    tags: ['quần áo', 'chiến dịch']
  },
  {
    id: 'food_support_expansion',
    title: 'Mở rộng bếp ăn nghĩa tình',
    date: '2025-09-25',
    summary: 'Thêm 200 suất ăn/ngày tại Đà Nẵng. Cần tình nguyện viên vận chuyển.',
    tags: ['thực phẩm', 'tình nguyện']
  }
];
...
function renderAnnouncements() {
  ...
  const items = announcements
    ...
    .map(a => {
      ...
      return `
      <article class="announcement-card" style="background:#fff;border-radius:12px;padding:16px;box-shadow:0 8px 24px rgba(0,0,0,0.08);border:1px solid #eee;">
        <div style="font-size:12px;color:#888;margin-bottom:6px;">${date}</div>
        <h3 style="margin:4px 0 8px 0;color:#ea5647;font-size:18px;">${a.title}</h3>
        <p style="margin:0 0 10px 0;">${a.summary}</p>
        <div style="display:flex;gap:6px;flex-wrap:wrap;">
          ${a.tags.map(t => `<span style=\"background:#f5f5f5;padding:4px 8px;border-radius:999px;font-size:12px;color:#555;\">#${t}</span>`).join('')}
        </div>
      </article>`;
    })
    .join('');
...
function appendMessage(text, sender = 'user') {
  ...
  if (sender === 'user') {
    // Avatar placeholder cho người dùng (hình tròn đơn giản)
    const avatar = document.createElement('div');
    avatar.className = 'message-avatar user-avatar';
    avatar.innerHTML = '<i class="fas fa-user"></i>';
    msg.appendChild(avatar);
  } else {
    // Avatar ảnh cho nhân viên tư vấn
    const avatar = document.createElement('img');
    avatar.src = 'nhanvien tu van.jpg';
    avatar.alt = 'Nhân viên tư vấn';
...
async function sendToGemini(userMessage) {
  try {
    ...
    // Lấy 4 lượt trao đổi gần nhất làm ngữ cảnh hội thoại
    const recentHistory = chatHistory.slice(-8).map(h => ({
      role: h.role === 'bot' ? 'model' : 'user',
      parts: [{ text: h.text }]
    }));

    // Hướng dẫn hệ thống để AI thành trợ lý hỗ trợ từ thiện
    const systemInstruction = `
Bạn là trợ lý AI cho một trang web từ thiện tại Việt Nam. Vai trò: tư vấn, hướng dẫn, giải đáp thắc mắc về:
- Hiến máu, quyên góp quần áo, hỗ trợ thực phẩm, giáo dục, y tế, khẩn cấp, quyên góp tiền.
- Chỉ dẫn người dùng cách đăng ký, liên hệ, và tìm trung tâm hỗ trợ.
Nguyên tắc trả lời: ngắn gọn, lịch sự, chính xác, bước‑bước rõ ràng, dùng bullet khi phù hợp, tránh nói chung chung. Nếu câu hỏi ngoài phạm vi, trả lời trung thực và gợi ý chủ đề phù hợp.
Ngôn ngữ trả lời: ${languageLabel}.
Ngữ cảnh trang: ${siteContext}
`;

    // Nếu người dùng hỏi về thông báo/tin tức, rút trích dữ liệu từ announcements để AI tóm tắt
    const announcementBrief = (() => {
      const q = userMessage.toLowerCase();
      const keywords = ['thông báo', 'tin mới', 'tin tức', 'cập nhật', 'news', 'announcement'];
      if (!keywords.some(k => q.includes(k))) return '';
      const latest = announcements
        .slice()
        .sort((a,b) => new Date(b.date) - new Date(a.date))
        .slice(0, 5)
        .map(a => `- ${new Date(a.date).toLocaleDateString('vi-VN')}: ${a.title} — ${a.summary}`)
        .join('\n');
    ...
    const contents = [
      ...
      ...recentHistory,
      { role: 'user', parts: [{ text: userMessage + announcementBrief }] }
    ];
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Render announcements sau khi DOM sẵn sàng
  renderAnnouncements();
});
...
// Money donation modal event listeners
document.addEventListener('DOMContentLoaded', function() {
  const moneyDonationModal = document.getElementById('moneyDonationModal');
  const closeMoneyDonation = document.getElementById('closeMoneyDonation');
  const moneyDonationForm = document.getElementById('moneyDonationForm');

  if (closeMoneyDonation && moneyDonationModal) {
    closeMoneyDonation.onclick = function() {
      moneyDonationModal.style.display = 'none';
      document.body.style.overflow = 'auto';
    };
  }

  if (moneyDonationForm) {
    moneyDonationForm.onsubmit = function(e) {
      e.preventDefault();
      handleMoneyDonationForm(e);
    };
  }

  // Close money donation modal when clicking outside
  if (moneyDonationModal) {
    window.addEventListener('click', function(event) {
      if (event.target === moneyDonationModal) {
        moneyDonationModal.style.display = 'none';
        document.body.style.overflow = 'auto';
      }
    });
...
// User account modal event listeners
document.addEventListener('DOMContentLoaded', function() {
  const userAccountModal = document.getElementById('userAccountModal');
  const closeUserAccount = document.getElementById('closeUserAccount');

  if (closeUserAccount && userAccountModal) {
    closeUserAccount.onclick = function() {
      userAccountModal.style.display = 'none';
      document.body.style.overflow = 'auto';
    };
  }

  // Close user account modal when clicking outside
  if (userAccountModal) {
    window.addEventListener('click', function(event) {
...
function handleMoneyDonationForm(e) {
  const formData = new FormData(e.target);
  const data = Object.fromEntries(formData.entries());
  
  // Validate required fields
  if (!data.donorName || !data.donorPhone || !data.donationAmount || !data.paymentMethod) {
  ...
  const donation = {
    ...
    donationDate: new Date().toLocaleString('vi-VN'),
    status: 'pending'
  };
  ...
  // Log donation data
  console.log('Thông tin quyên góp:', donation);
  ...
  console.log('Cảm ơn bạn đã quyên góp! Chúng tôi sẽ liên hệ sớm nhất.');
...
function openUserAccountModal() {
  ...
  if (userAccountModal) {
    ...
    if (user.name) {
      ...
      userDetailEmail.textContent = user.email || 'user@example.com';
      userPhone.textContent = user.phone || '0123456789';
      userRegisterDate.textContent = user.registerDate || '01/01/2024';
    } else {
      // Thông tin mặc định
      userDisplayName.textContent = 'nguyễn khắt thuận';
      userEmail.textContent = 'user@example.com';
      userFullName.textContent = 'Nguyễn Khắt Thuận';
      userDetailEmail.textContent = 'user@example.com';
      userPhone.textContent = '0123456789';
      userRegisterDate.textContent = '01/01/2024';
    }
...
function logoutUser() {
  ...
  // Hiển thị nút đăng nhập/tạo tài khoản và ẩn profile
  ...
  // Cập nhật UI để hiển thị nút đăng nhập
  updateUserInterface();
...
function updateUserInterface() {
  ...
  if (user.name) {
    // User đã đăng nhập - ẩn auth buttons, hiển thị profile
    if (authButtons) authButtons.style.display = 'none';
    if (userProfileNav) userProfileNav.style.display = 'flex';
    
    if (userBox) {
      userBox.innerHTML = `
        <span onclick="openUserAccountModal()" style="cursor:pointer;">
          <i class="fas fa-user-circle"></i> ${user.name}
        </span>
      `;
    }
  } else {
    // User chưa đăng nhập - hiển thị auth buttons, ẩn profile
    if (authButtons) authButtons.style.display = 'flex';
    if (userProfileNav) userProfileNav.style.display = 'none';
...
function editUserProfile() {
  // Tạo modal chỉnh sửa profile
  const editModal = document.createElement('div');
  editModal.className = 'modal';
  editModal.style.display = 'flex';
  editModal.innerHTML = `
    <div class="modal-content" style="max-width:500px;">
      <span class="close" onclick="this.parentElement.parentElement.remove()" style="float:right;cursor:pointer;">&times;</span>
      ...
      <form class="modal-form" id="editProfileForm">
        <div class="form-group">
          ...
          <input type="text" id="editName" name="editName" placeholder="Nhập họ và tên" required>
        </div>
        <div class="form-group">
          <label for="editEmail">Email *</label>
          <input type="email" id="editEmail" name="editEmail" placeholder="Nhập email" required>
        </div>
        <div class="form-group">
          <label for="editPhone">Số điện thoại</label>
          <input type="tel" id="editPhone" name="editPhone" placeholder="Nhập số điện thoại">
        </div>
        <div class="form-group">
          <label for="editAddress">Địa chỉ</label>
          <textarea id="editAddress" name="editAddress" placeholder="Nhập địa chỉ" rows="3"></textarea>
        </div>
  ...
  const editPhone = editModal.querySelector('#editPhone');
  const editAddress = editModal.querySelector('#editAddress');
  ...
  const editForm = editModal.querySelector('#editProfileForm');
  if (editForm) {
...
function handleEditProfile(e) {
  ...
  // Validate required fields
  if (!data.editName || !data.editEmail) {
    console.log('Vui lòng điền đầy đủ thông tin bắt buộc');
    return;
  }
  ...
  user.phone = data.editPhone || '';
  user.address = data.editAddress || '';
  user.updateDate = new Date().toLocaleString('vi-VN');
...
function openShareForm() {
  ...
  shareModal.innerHTML = `
    <div class="modal-content" style="max-width:600px;">
      <span class="close" onclick="this.parentElement.parentElement.remove()" style="float:right;cursor:pointer;">&times;</span>
      <h3 style="color:#28a745;margin-bottom:20px;">
        <i class="fas fa-share-alt"></i> Chia Sẻ Hoàn Cảnh
      </h3>
      
      <form class="modal-form compact-form" id="shareForm">
        <div class="form-row">
          <div class="form-group">
            <input type="text" id="shareName" name="shareName" placeholder="Tên của bạn hoặc người cần giúp đỡ *" required>
          </div>
          <div class="form-group">
            <textarea id="shareStory" name="shareStory" placeholder="Chia sẻ hoàn cảnh, câu chuyện... *" rows="4" required></textarea>
          </div>
        </div>
        <div class="form-actions">
          <button type="submit" class="btn-primary">
...
function handleShareForm(e) {
  ...
  const share = {
    id: Date.now(),
    name: data.shareName,
    story: data.shareStory || '',
    contact: data.shareContact || '',
    shareDate: new Date().toLocaleString('vi-VN'),
    status: 'pending'
  };
  ...
  // Log share data
  console.log('Thông tin chia sẻ:', share);
  ...
  // Show success message
  console.log('Cảm ơn bạn đã chia sẻ! Chúng tôi sẽ liên hệ sớm nhất.');
  
  // Reset form
  e.target.reset();
}

// Add event listeners for auth buttons when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
  ...
  adjustFontSizeForName();
});

// Function to auto-adjust font size based on name length
function adjustFontSizeForName() {
  const nameElement = document.querySelector('.user-name-small-nav');
  if (!nameElement) return;
  
  const name = nameElement.textContent.trim();
  const containerWidth = nameElement.offsetWidth;
  const maxWidth = nameElement.parentElement.offsetWidth - 20; // 20px padding
